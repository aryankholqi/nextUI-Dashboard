...
export default function Sidebar() {
  ...
  return (
    <div
      className={`bg-black shadow-none px-5 py-4 h-[93.5dvh]  transition-all duration-[250ms] ${isInLaptopView && !isSidebarExpanded ? "rounded-full" : "rounded-[40px]"} ${isSidebarExpanded ? "toggleSidebar" : "w-max"} ${!isInLaptopView ? "absolute z-50" : ""
        } ${!isInLaptopView && isSidebarOpen
          ? "left-5 w-max sm:w-[40%]"
          : ""
        } ${!isInLaptopView && !isSidebarOpen ? "-left-96" : ""}`}
    >
      <div className="flex flex-col justify-between h-full relative">
        <MainTooltip content="Expand">
          <span className={`${isInLaptopView ? "inline-block" : "hidden"} absolute -right-8 top-[50px] cursor-pointer transition-all duration-250 ${isSidebarExpanded ? "rotate-180" : "rotate-0"}`} onClick={expandSidebarToggle}>
            <ExpandRightIcon />
          </span>
        </MainTooltip>
