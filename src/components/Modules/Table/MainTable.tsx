...
const rowsOfPage = [{ key: 5, label: "5" }, { key: 10, label: "10" }, { key: 15, label: "15" }]
...
export default function MainTable() {
    ...
    const renderCell = useCallback((currency: currenyMarketsProps, columnKey: Key) => {
        ...
        switch (columnKey) {
            ...
            case "amount":
                return (
                    <p>{formatPrice(currency.amount)}</p>
                );
            case "date":
                return (

                    <p>{convertTimestampsToDate(currency.date)}</p>
                );
            case "status":
                return (
                    <Chip className="capitalize" color={statusColorMap[currency.status]} size="sm" variant="flat">
                        {cellValue}
                    </Chip>
                )
            case "actions":
    ...
    const topContent = useMemo(() => (
        <div className="flex flex-col gap-4">
            <div className="flex flex-wrap sm:flex-nowrap justify-between gap-3 items-end">
                <Controller name="search" control={control} render={({ field: { onChange, value } }) => (
                    <Input
                        isClearable
                ...
                <div className="flex items-center flex-wrap sm:flex-nowrap gap-3">
                    <Controller name="rowsPerPage" defaultValue={5} control={control} render={({ field: { onChange } }) => (
                        <Select
                            items={rowsOfPage}
                            label={"Rows Per Page"}
                    ...
                    <Controller name="visibleColumns" control={control} render={({ field: { onChange, value } }) => (
                        <Dropdown>
                            <DropdownTrigger>
